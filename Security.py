...
class C2C():
	...
	def get_trading_volume_yesterday(self, rank):
		...
		不减去已刷数量，只适合第一次刷
		'''
		log.info(u'交易对{0},从昨日数据获得待刷量，但未检测现已刷数量，因此只适合第一次刷，请注意！!'.format(self.trade).encode(code))
		...
		count = len(ranks)
		if count > rank:
			trading_volume = ranks[rank]['tradeVolume']
		else:
			try:
...
def exp(job):
	(demo, rank, yesterday,uid)= job
	# 初始化状态
	check_security(demo,uid)
	if int(yesterday):
	...
	while i < trading_volume:
		...
		demo.sell(interval, price)
		entrustId = demo.get_entrustId(uid)
		demo.buy(interval, price,entrustId)
		tradeID = demo.get_succ_id(uid,uid)
		if not len(tradeID):
			log.warn(u'此次交易被截胡，未完成刷单,继续下一次'.encode(code))
			i -= 2*interval
			break
		for i in tradeID:
			if demo.pay(i):
