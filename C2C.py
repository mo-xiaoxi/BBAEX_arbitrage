...
class C2C():
	...
	def get_succ_id_init(self, ID):
		...
		for d in data['data']['rows']:
			...
			if int(d['buyId']) == int(ID) or int(d['sellId']) == int(ID):
				if int(d['coinId'])== int(self.MarketCoinId):
...
def check_security(demo,uid):
	...
	'''
	demo.cancel()
	tradeID = demo.get_succ_id_init(uid)
	for i in tradeID:
		demo.pay(i)
		demo.confirm(i)
	return True
...
def exp(demo, rank, yesterday,uid):
	# 初始化状态
...
def exit():
	...
	jobs = []
	for demo in demos:
		jobs.append(gevent.spawn(check_security,demo,uid))
	gevent.joinall(jobs)
...
if __name__=='__main__':
	...
	while True:
		...
		for trade in config['Trades']:
			...
			demos.append(demo)
			jobs.append(p.spawn(exp,demo, trade['rank'], trade['yesterday'], uid))
